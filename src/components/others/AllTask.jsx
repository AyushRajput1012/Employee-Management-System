  ...
  return (
    <div id="Alltasks" className='bg-[#1C1C1C] p-5 rounded mt-5  '>
      ...
      <div id='Alltasks' className=' '>
        ...
        const taskCounts = getTaskCountsFromTasks(elem.tasks || [])
        return <div key={idx} className='border-2 border-emerald-300 mb-2  py-2 px-4 flex justify-between rounded'>
        <h2 className='w-1/5   '>{elem.firstName}</h2>
        <h3 className='tetx-lg font-medium w-1/5 text-blue-600   '>{taskCounts.newTask}</h3>
        <h5 className='tetx-lg font-medium w-1/5 text-yellow-500   '>{taskCounts.active}</h5>
        <h5 className='tetx-lg font-medium w-1/5 text-green-600   '>{taskCounts.completed}</h5>
        <h5 className='tetx-lg font-medium w-1/5 text-red-600   '>{taskCounts.failed}</h5>
      </div>
      })}
      </div>
